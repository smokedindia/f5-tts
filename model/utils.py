...
def get_tokenizer(dataset_name, tokenizer: str = "pinyin"):
    ...
    if tokenizer in ["pinyin", "char"]:
        with open(
            f"data/{dataset_name.replace('_heavy', '')}_{tokenizer}/vocab.txt", "r", encoding="utf-8"
        ) as f:
            vocab_char_map = {}
...
def convert_char_to_pinyin(text_list, lang='zh', polyphone=True):
    if lang == 'en':
        import g2p_en
        g2p = g2p_en.G2p()
        return [g2p(text) for text in text_list]
    elif lang == 'ko':
        from jamo import h2j, j2hcj
        return [list(j2hcj(h2j(c))) for c in text_list]
